...
def test_pycomm3_transport_translates_service_and_errors(monkeypatch):
    captured_service = []

    class _Error:
        status = 0x15
        extended_status = [0x0107]
    ...
    class _StubDriver:
        ...
        def generic_message(self, **kwargs):
            captured_service.append(kwargs.get("service"))
            return _StubTag(value=b"", error=_Error())
    ...
    request = ServiceRequest(
        ...
        tag_path="assembly/101",
        metadata={"class": "0x04", "instance": "101", "attribute": "3"},
    )

    response = transport.send(request)

    assert captured_service == [0x0E]
    assert response.status.startswith("Too much data")
